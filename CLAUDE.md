# Claude Code Project Instructions — thiserror-kotlin

## Project Overview

This is **thiserror-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`thiserror`](https://crates.io/crates/thiserror).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/thiserror/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/thiserror>
